...
def build_options(vault, cfg: dict, resume: str | None = None, msg=None) -> ClaudeAgentOptions:
    ...
    footer = (
        ...
        f"(You run on the Claude Code harness, which serves Claude models only. "
        f"Runtime settings live at {SETTINGS} — currently model = "
        f"\"{cfg['model']}\", language = \"{cfg['language']}\". When the user asks "
        f"to switch to a CLAUDE model (\"default\", an alias sonnet/opus/haiku, or "
        f"a full claude-* id) or reply language (\"en\"/\"zh\"), Edit that file and "
        f"confirm; it applies from the next message. This harness CANNOT run "
        f"non-Claude models (ChatGPT/OpenAI, Gemini, etc.) — if the user wants one, "
        f"tell them to restart the bot with `wcob run-api`, and do NOT put a "
        f"non-Claude model (anything with a provider: prefix) in the settings file, "
